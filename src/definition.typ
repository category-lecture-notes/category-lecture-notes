...
#definition[
    (Category). A category is the followin things:
    - A collection of objects,
    - A collection of morphism, or arraws, such that each morphism $f$ has a _domain_ and _codomain_. A morphism $f$ of domain $x$ and codomain $y$ will be written $f : x -> y$. We also ask the following properties:
...
#example[
    ...
    - #Set, the caregory of sets, and arrows as functions,
    ...
    - any (pre)ordered set $(P,<=)$ is a category, names $P$ with objects the element of $P$, and between $(x,y)$, a unique $f : x -> y$ iff $x <= y$,
...
#definition[
    ($dual(cat)$). Let $cat$ be a category.
    We call $dual(cat)$ the category with the same objects, and for arrows, a morphism $dual(f) : y -> x$ for each morphism $f : x -> y$.
]

#example[
    It is not outright clear why this construction is important, but it serves to naturally express some concepts, as we will see in, for example: Given a poset $cat := (P,<=)$, $dual(cat)$ is $(P,>=)$.
]
...
#definition[
    (Monomorphism, epimorphism and isomorphism). Let $cat$ be a category.
...
We say that a category $cat$ that only contains isomorphisms is a _groupoid_.
...
#definition[
    (Hom-set, small and locally small categories). Let $cat$ be a category. For all $x, y in cat$, we name $cat(x, y)$ (also written as $"Hom"_(cat)(x, y)$) the collection of arrows between $x$ and $y$.

    We say that a category is _small_ if it has only a set worth of objects.

    We say that a category is locally small if, for all $x, y in cat$, $cat(x, y)$ is a set.
]

As for now, the importance of smallness and local smallness don't semm relevant, but the importance of local smallness will be highlighted in the next chapter.
